...
class TradingModel:
    ...
    def train(self, X_train, y_train, epochs=50, batch_size=32, validation_split=0.2):
        ...
        """
        # Verificar si hay NaN en los datos de entrenamiento
        if np.isnan(X_train).any():
            logger.warning(f"Se detectaron valores NaN en los datos de entrenamiento. Reemplazando con 0.")
    ...
    def predict(self, X):
        ...
        try:
            ...
            if np.isnan(X).any():
                logger.warning(f"Se detectaron valores NaN en los datos de entrada. Reemplazando con 0.")
                X = np.nan_to_num(X, nan=0.0)
            
            predictions = self.model.predict(X)
            
            # Verificar si hay NaN en las predicciones
            if np.isnan(predictions).any():
                logger.warning(f"Se detectaron valores NaN en las predicciones. Reemplazando con 0.5 (neutral).")
            ...
            # Retornar predicciones neutrales en caso de error
