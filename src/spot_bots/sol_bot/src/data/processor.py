...
class DataProcessor:
    ...
    def add_indicators(self, df):
        ...
        try:
            ...
            df_with_indicators['sma_20'] = df_with_indicators['close'].rolling(window=40).mean()  # 20 horas
            df_with_indicators['sma_50'] = df_with_indicators['close'].rolling(window=100).mean()  # ~2 días
            df_with_indicators['sma_200'] = df_with_indicators['close'].rolling(window=400).mean()  # ~8 días
            ...
            avg_gain = gain.rolling(window=28).mean()
            ...
            ema_12 = df_with_indicators['close'].ewm(span=24, adjust=False).mean()
            ema_26 = df_with_indicators['close'].ewm(span=52, adjust=False).mean()
            df_with_indicators['macd'] = ema_12 - ema_26
            df_with_indicators['macd_signal'] = df_with_indicators['macd'].ewm(span=18, adjust=False).mean()
            df_with_indicators['macd_histogram'] = df_with_indicators['macd'] - df_with_indicators['macd_signal']
            
            # Calcular Bandas de Bollinger
            df_with_indicators['bollinger_mid'] = df_with_indicators['close'].rolling(window=40).mean()
