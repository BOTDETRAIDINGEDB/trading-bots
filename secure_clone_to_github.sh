...
# Con protecciones adicionales para información sensible
...
echo -e "${YELLOW}4. Copiando archivos del bot...${NC}"
...
# 5. Copiar archivos de la API
echo -e "${YELLOW}5. Copiando archivos de la API...${NC}"
...
SENSITIVE_PATTERNS=(
    "BINANCE_API_KEY=[a-zA-Z0-9]+"
    "BINANCE_API_SECRET=[a-zA-Z0-9]+"
    "TELEGRAM_BOT_TOKEN=[0-9]+:[a-zA-Z0-9_-]+"
    "TELEGRAM_CHAT_ID=[0-9-]+"
    "API_KEY=[a-zA-Z0-9]+"
    "JWT_SECRET=[a-zA-Z0-9]+"
    "API_SECRET_KEY=[a-zA-Z0-9]+"
    "password"
    "secret"
    "token"
    "apikey"
...
# Buscar patrones sensibles en todos los archivos
echo -e "${YELLOW}   Buscando información sensible...${NC}"
for pattern in "${SENSITIVE_PATTERNS[@]}"; do
    echo -e "${YELLOW}   Buscando: $pattern${NC}"
    # Buscar en el directorio de bots
    grep -r "$pattern" "$TEMP_DIR/trading-bots" --include="*" 2>/dev/null | while read -r file; do
        echo -e "${RED}   ¡Encontrada información sensible en: $file${NC}"
    done
    
    # Buscar en el directorio de API
    grep -r "$pattern" "$TEMP_DIR/trading-bots-api" --include="*" 2>/dev/null | while read -r file; do
        echo -e "${RED}   ¡Encontrada información sensible en: $file${NC}"
    done
done

# Eliminar todos los archivos .env y crear versiones de ejemplo
